...
### Prerequisites
*   The `tinymist` language server executable must be installed and accessible. It should ideally be on the system `PATH`. The tests, as part of the plugin, will likely use the same mechanism to find the executable as the plugin itself (e.g., via `TinymistLspStreamConnectionProvider.kt`, which might check a hardcoded path, system `PATH`, or future plugin settings).

### Command
*   To run the LSP integration tests, use the following Gradle command from the root of the `tinymist` repository:
    ```bash
    ./gradlew :editors:intellij:test
    ```
*   Alternatively, if your terminal is already in the `editors/intellij` directory, you might be able to use:
    ```bash
    ./gradlew test
    ```

### Expected Outcome
*   The tests will configure a lightweight IntelliJ environment (often headless or using minimal UI components) to host the plugin and the LSP client.
*   The `LspIntegrationTestBase` class and its subclasses will then programmatically interact with the `tinymist` language server, sending LSP requests (e.g., for completion, hover, diagnostics) and asserting the responses.
*   Test results (success/failure, logs) will be generated by Gradle and typically found in `editors/intellij/build/reports/tests/test/`.

### Troubleshooting/Notes
*   **Server Not Found:** If the `tinymist` executable is not found by the test environment (based on how `TinymistLspStreamConnectionProvider` tries to locate it), tests that require a running LSP server will likely fail or be skipped. Ensure `tinymist` is compiled and accessible.
*   **Assertion Sensitivity:** The assertions in the integration tests (e.g., for specific diagnostic messages, hover content, completion item labels, or formatting results) are based on the current behavior of the `tinymist` LSP server. If the server's output for these features changes in future versions, these assertions may need to be updated to reflect the new expected behavior.
*   **Headless Environment:** Some tests might behave differently in a fully headless CI environment compared to a local development machine if they inadvertently rely on UI components that are not available. The test setup aims to mitigate this.
*   **Test Timeouts:** LSP interactions involve asynchronous operations. Tests include timeouts (e.g., 5-10 seconds) to await server responses. Slow server startup or response on a heavily loaded machine could cause sporadic timeout failures.